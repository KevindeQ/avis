...
namespace graphics
{
    render_device::render_device(const bool use_warp, const bool enable_debug) :
        ...
        d3d12_debug_controller{ nullptr }
    {
        ...
        init_hardware_device_d3d12(use_warp, enable_debug);
    }
    ...
    void render_device::init_hardware_device_d3d12(const bool use_warp, const bool enable_debug)
    {
    ...
    void render_device::enable_debug_layer()
    {
        if (d3d12_debug_controller)
        {
    ...
    void render_device::disable_debug_layer()
    {
    ...
    com_ptr<IDXGIAdapter1>
        render_device::get_hardware_adapter(com_ptr<IDXGIFactory4>& factory, bool request_high_perf_adapter)
    {
        com_ptr<IDXGIAdapter1> adapter{};
        ...
        if (SUCCEEDED(factory.as(factory6)))
        {
            ...
            {
                if (!SUCCEEDED(
                        factory6->EnumAdapterByGpuPreference(adapter_index, gpu_preference, IID_PPV_ARGS(&adapter))))
                {
                    break;
                }
